...
//NEW COMMENTS
//host mode (lab 6.2)
//SNDFIFO and RCVFIFO give access to the buffers
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//int XSpi_Transfer(XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount);
		//this function transfers the specified data on SPI bus
		//for every byte sent, one is received
		//for writing, we're sending data so the receiving buf is NULL
	BYTE write_buf[2] = {reg + 2, val};
	u8 read_buf[2];
	XSpi_SetSlaveSelect(&SpiInstance, 1);

	int result = XSpi_Transfer(&SpiInstance, write_buf, NULL , 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int result = XSpi_Transfer(&SpiInstance, write_buf, NULL , nbytes+1);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	u8 read_buf[2] = {};
		...
		int result = XSpi_Transfer(&SpiInstance, write_buf, write_buf ,2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	u8 write_buf[nbytes +1];
	write_buf[0] = reg;
	u8 read_buf[nbytes +1];

	int result = XSpi_Transfer(&SpiInstance, write_buf, write_buf , nbytes+1);
